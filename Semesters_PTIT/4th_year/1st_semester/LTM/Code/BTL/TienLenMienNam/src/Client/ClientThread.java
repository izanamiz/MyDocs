				...
				switch (tag) {
					case "ResponseDeckShuffled":
						...
						if (clientGameplay.isYourTurn == true) {
							...
							System.out.println("\n\n--------------------------------------------------------");
							TimerExample.openFrame(clientGameplay);
						...
						for (int i = 0; i < 13; i++) {
							...
							BufferedImage bufferdImage = ImageIO
									.read(new File(clientGameplay.deckShuffledList.get(i).getUrl()));
					...
					case "ResponseCardOnTable":
						...
						if (!clientGameplay.accountName.equals(responseCardOnTable.getAccountName())) {
							for (int j = 0; j < clientGameplay.nameOfAnotherPlayer.size(); j++) {
								...
								if (responseCardOnTable.getAccountName().equals(name)) {
									// kiểm tra client đánh bài lên là client nào trong danh sách các client trong
									// ván game
									...
									if (name.equals(clientGameplay.lblAnotherClientSlotOne.getText().toString())) {
										...
										clientGameplay.numberCardSlotOne
												.setText(String.valueOf(clientGameplay.numCardClientOne));
									...
									if (name.equals(clientGameplay.lblAnotherClientSlotTwo.getText().toString())) {
										...
										clientGameplay.numberCardSlotTwo
												.setText(String.valueOf(clientGameplay.numCardClientTwo));
									...
									if (name.equals(clientGameplay.lblAnotherClientSlotThree.getText().toString())) {
										...
										clientGameplay.numberCardSlotThree
												.setText(String.valueOf(clientGameplay.numCardClientThree));
						...
						for (int i = 0; i < responseCardOnTable.getDeckImport().size(); i++) {
							...
							for (int y = 0; y < clientGameplay.cardOnTable.size(); y++) {
								System.out.println(
										"clientthread say, it geted: " + clientGameplay.cardOnTable.get(y).getPoint());
							}
						}
						...
						for (int y = 0; y < clientGameplay.cardOnTable.size(); y++) {
							System.out.println(
									"part 2 clientthread say, it geted: "
											+ clientGameplay.cardOnTable.get(y).getPoint());
						}
						// chọn lựa và lưu trữ đối tượng của imageIconOnTableList
						for (int i = 0; i < clientGameplay.cardOnTable.size(); i++) {
							BufferedImage bufferdImage = ImageIO
									.read(new File(clientGameplay.cardOnTable.get(i).getUrl()));
					...
					case "ResponseSendMessenger":
						if (dataPacket instanceof ResponseSendMessenger) {
							...
							if (responseSendMessenger.getContentMessenger()
									.contains("@private-" + clientGameplay.getAccountName())) {
								isPrivateMessengerForYou = true;
							}

							if (!isPrivateMessenger) {
								String string = responseSendMessenger.getAccountName() + ": "
										+ responseSendMessenger.getContentMessenger() + "\n";
							...
							if (isPrivateMessengerForYou) {
								String string = responseSendMessenger.getAccountName() + " gửi một tín nhắn riêng: "
										+ responseSendMessenger.getContentMessenger() + "\n";
					...
					case "ResponseRedo":
						ResponseRedo responseRedo = (ResponseRedo) dataPacket;
						String string = responseRedo.getAccountName() + ": Yeu cau di lai \n";
						clientGameplay.logMessenger.append(string);
						clientGameplay.textPaneDisplayRequestRedo.setText(clientGameplay.logMessenger.toString());
						break;

					case "ResponseAccept":

						// Xóa các lá bài đang hiển thị trên bàn
						clientGameplay.cardOnTable.clear();
						clientGameplay.imageIconOnTableList.clear();

						// Import lá bài của người chơi hiện tại vào bàn
						for (Card playerCard : clientGameplay.originalCardList) {
							clientGameplay.cardOnTable.add(playerCard);

							try {
								BufferedImage bufferdImage = ImageIO.read(new File(playerCard.getUrl()));
								Image imageScaled = bufferdImage.getScaledInstance(61, 90, Image.SCALE_SMOOTH);
								ImageIcon imageIcon = new ImageIcon(imageScaled);
								clientGameplay.imageIconOnTableList.add(imageIcon);
							} catch (IOException e) {
